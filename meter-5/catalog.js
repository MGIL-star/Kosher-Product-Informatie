(function () {
  ...
  function render() {
    ...
    if (!visible.length) {
      ...
      document.querySelector('#empty-copy').textContent = hasProducts ? 'Probeer een andere zoekterm of kies een ander filter.' : 'Meter 5 staat klaar voor de echte productgegevens.';
    }
  }
  ...
  function openProduct(product) {
    ...
    dialogContent.innerHTML = `<div class="dialog-product">
      ...
      ${product.ean ? `<section class="ean"><h3>Barcode / EAN</h3><p>${escapeHtml(product.ean)}</p>${product.barcodeNeedsCheck ? '<p>Nog te controleren op de verpakking.</p>' : ''}</section>` : ''}
    </div>`;
    dialog.showModal();
  }
