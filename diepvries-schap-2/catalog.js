(function () {
  ...
  function render() {
    ...
    if (!visible.length) {
      ...
      document.querySelector('#empty-copy').textContent = hasProducts ? 'Probeer een andere zoekterm of kies een ander filter.' : 'Diepvries schap 2 staat klaar voor de echte productgegevens.';
    }
  }
  ...
  function openProduct(product) {
    ...
    dialogContent.innerHTML = `<div class="dialog-product">
      ...
      <section><h3>Ingrediënten</h3><p>${escapeHtml(product.ingredients)}</p>${product.ingredientNote ? `<p class="verification-copy">${escapeHtml(product.ingredientNote)}</p>` : ""}</section>
      ...
      ${product.kosher ? `<section class="hechsher"><h3>Hechsher</h3><p>${escapeHtml(hechsherText(product.kosher + ((product.kosherStatus === 'Parve' || /\(Parve\)/i.test(product.note || '')) && !/parve/i.test(product.kosher) ? ' (Parve)' : '')))}</p>${product.kosherNote ? `<p class="verification-copy">${escapeHtml(product.kosherNote)}</p>` : ""}</section>` : ''}${product.preparation ? `<section><h3>Bereiding</h3><p>${escapeHtml(product.preparation)}</p></section>` : ''}${product.note ? `<section><h3>Productinformatie</h3><p>${escapeHtml(product.note)}</p></section>` : ''}
    </div>`;
    dialog.showModal();
  }
