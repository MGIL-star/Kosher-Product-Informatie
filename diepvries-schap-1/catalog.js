(function () {
  ...
  function productImage(product, large = false) {
    ...
    if (product.image) {
      return `<div class="product-visual${sizeClass}"><img data-product-image="${product.id}" src="${escapeHtml(product.image)}" alt="${product.id >= 6 ? 'Illustratieve productpresentatie: ' : ''}${escapeHtml(product.name)}" loading="lazy"></div>`;
    }
  ...
  function render() {
    ...
    if (!visible.length) {
      ...
      document.querySelector('#empty-copy').textContent = hasProducts ? 'Probeer een andere zoekterm of kies een ander filter.' : 'Diepvries schap 1 staat klaar voor de echte productgegevens.';
    }
  }
  ...
  function openProduct(product) {
    const allergens = product.allergens.length
      ...
      : (product.allergensConfirmedAbsent ? '<span class="allergen-free">Geen</span>' : '<span>Nog te controleren op de verpakking</span>');
    dialogContent.innerHTML = `<div class="dialog-product">
      ${productImage(product, true)}
      <p class="image-disclaimer">${product.id >= 6 ? 'Illustratieve productpresentatie.' : 'Afbeelding kan afwijken van de actuele verpakking.'}</p>
