(function () {
  ...
  function productImage(product, large = false) {
    ...
    if (product.image) {
      return `<div class="product-visual${sizeClass}" data-photo-id="${product.id}"><img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.brand)} ${escapeHtml(product.name)}" loading="lazy"></div>`;
    }
  ...
  function render() {
    ...
    const visible = products.filter(product => {
      ...
      const haystack = normalize(`${product.brand} ${product.name} ${product.englishName || ''} ${product.variant} ${product.ingredients} ${product.ean || ''} ${Object.values(product.productInfo || {}).join(' ')}`);
    ...
    if (!visible.length) {
      ...
      document.querySelector('#empty-copy').textContent = hasProducts ? 'Probeer een andere zoekterm of kies een ander filter.' : 'Meter 11 staat klaar voor de echte productgegevens.';
    }
  }

  function openProduct(product) {
    const allergens = product.allergens === null
      ? '<span class="allergen-free">Nog niet bevestigd</span>'
    ...
    dialogContent.innerHTML = `<div class="dialog-product">
      ...
      <p class="image-disclaimer">${product.imageEdited ? 'Met AI bewerkte productfoto; kleine etikettekst kan afwijken.' : product.imageIllustration ? 'Illustratie van het product, geen verpakkingsfoto.' : 'Afbeelding kan afwijken van de actuele verpakking.'}</p>
      ...
      ${Object.keys(product.productInfo || {}).length ? `<section><h3>${product.isWine ? 'Productinformatie / wijnstijl' : 'Productinformatie'}</h3>${Object.entries(product.productInfo).map(([label, value]) => `<p><strong>${escapeHtml(label)}:</strong> ${escapeHtml(value)}</p>`).join('')}</section>` : ''}
      ${product.ingredients ? `<section><h3>${product.ingredientsPartial ? 'Bekende ingrediënten' : 'Ingrediënten'}</h3><p>${window.KPI_emphasizeAllergens(product.ingredients)}</p>${product.ingredientsPartial ? `<p class="ingredient-check">${escapeHtml(product.ingredientsNote || "Dit is een onvolledige opsomming. De volledige ingrediëntenlijst volgt na etiketcontrole.")}</p>` : ''}</section>` : product.detailsPending ? '' : '<section><h3>Ingrediënten</h3><p>Nog te controleren op het etiket.</p></section>'}
      ${product.nutrition ? `<section><h3>Voedingswaarden Per 100 g</h3>${Object.entries(product.nutrition).map(([label, value]) => `<p><strong>${escapeHtml(label)}:</strong> ${escapeHtml(value)}</p>`).join('')}</section>` : ''}
      ${product.detailsPending ? '' : `<section><h3>Allergenen</h3><div class="allergen-list">${allergens}</div>${product.allergenNote ? `<p>${escapeHtml(product.allergenNote)}</p>` : ''}</section>`}
      ${product.mayContain ? `<section class="may-contain"><p><strong>Kan bevatten:</strong> ${escapeHtml(product.mayContain)}</p></section>` : ''}
      ...
      ${product.ean ? `<section class="ean"><h3>Barcode / EAN</h3><p>${escapeHtml(product.ean)}</p>${product.barcodeNeedsCheck ? `<p>${escapeHtml(product.barcodeNote || 'Nog te controleren op de verpakking.')}</p>` : ''}</section>` : ''}
    </div>`;
    dialog.showModal();
  }
